export default class GameCtrl {
  ...
  _checkIfCheck() {
    ...
    const isKingInDanger = positions =>
      positions.join() === enemyKingPosition.join();
  ...
  _findKingMovesWhenCheck() {
    const legalMoves = [];
    const king = this._boardModel[this._check.onKingPosition[0]][
      this._check.onKingPosition[1]
    ];
  ...
  _filterEnemyKingPosition(moves) {
    let kingPosition;
    const oppositeKingColor =
      this._markedFigure._side === "white" ? "black" : "white";
    ...
    this._boardModel.forEach(row => {
      row.forEach(figure => {
        if (
          figure &&
          figure.name === "king" &&
          figure._side === oppositeKingColor
        ) {
  ...
  _handleMove(position) {
    ...
    //Sprawdza czy ruch, który ma być wykonany jest roszadą i od razu przypisuje jaka to ma być roszada
    let castling = "";
    for (let i = 0; i < moves.length; i++) {
      if (moves[i][2]) castling = moves[i][2];
    }
    ...
    if (castling) {
      ...
      this._markedFigure.move(position, this._boardModel);
      this._boardView.movePiece(oldPosition, this._markedFigure);
      if (castling === "smallWhiteCastling") {
        this._boardModel[7][7].move([7, 5], this._boardModel);
        this._boardView.movePiece([7, 7], this._boardModel[7][5]);
      } else if (castling === "smallBlackCastling") {
        this._boardModel[0][7].move([0, 5], this._boardModel);
        this._boardView.movePiece([0, 7], this._boardModel[0][5]);
      } else if (castling === "bigWhiteCastling") {
        this._boardModel[7][0].move([7, 3], this._boardModel);
        this._boardView.movePiece([7, 0], this._boardModel[7][3]);
      } else if (castling === "bigBlackCastling") {
        this._boardModel[0][0].move([0, 3], this._boardModel);
        this._boardView.movePiece([0, 0], this._boardModel[0][3]);
      }
      this._afterMoveOrAttack();
    }
  ...
  _initTimers(gameDurationInMinutes) {
    ...
    const gameDurationTimeStamp =
      currentTime + gameDurationInMinutes * 60 * 1000 - currentTime;
